<template>
    <div class="match-detailpage" ref="matchDetailPage">
        <!-- <v-navbar class="navbar-demo2" setBgColor="#fff">赛事详情</v-navbar> -->
        <div class="wrap" ref="comentList">
            ...
            <div class="matchDetail-user-msg">
                <div class="user-avatar">
                    <img :src='commentHeadImgUrl(detailContent.applyUpNum)'  v-if="detailContent.applyUpEntrance !== 2">
                    <img class="offical-head-img" src="../../assets/images/group.png" v-else />
                    <div class="head-decorate" v-if="detailContent.headImageType !== 0">
                        <img class="head-img" :src="headDecorate(detailContent.headImageType)"/>
                    </div>
                </div>
                <div class="user-msg">
                  <span>{{detailContent.applyUpName}}</span>
            ...
            <div class="video-wrap">
                <video ref="videoPlayer" :src="matchDetailVideoUrl" :poster="detailContent.coverUrl" controls preload="auto"></video>
            ...
            <div class="like-block" @click="like()" v-if="isShow" v-show="isShowBtnLike">
                <img src="../../assets/images/love.png">
                <span>{{detailContent.likeNum}}</span>

            </div>
            <div class="like-block" style="border:1.5px solid #f39e13;" @click="cancleLike()" v-if="!isShow" v-show="isShowBtnLike">
        ...
        <!-- 评论 -->
        <div class="comment-container" v-show="isShowBtnLike">
...
export default {
    ...
    methods:{
        ...
        getMatchDetail:function(uuid){
            let that = this;
            this.$axios.post('match/queryVedioDetailsInfo',{"uuid":uuid})
            .then(function (response) {
                var data = response.data;
                var result = data.result;
                that.detailContent = result;
                that.matchDetailVideoUrl = result.videoUrl;
                if(result.publishStatus === 0 || result.publishStatus === 2) {
                    that.isShowBtnLike = false;
                }
        ...
        getCommentData() {
            let that = this;
            this.$axios.post('match/selectComment',{
                uuid: this.uuid,
        ...
        loadMoreComment() {
            ...
            comentListDom.addEventListener('scroll', function() {
                ...
                if(offsetHeight + scrollTop > scrollHeight -20) {
                    ...
                    that.$axios.post('match/selectComment',{
                        uuid: that.uuid,
        ...
        like:function(){
            ...
            let uuid = this.uuid;
            let that =this;
             this.$axios.post('match/likeVideo',{"uuid":uuid})
            .then(function (response) {
        ...
        cancleLike:function(){
            ...
            let uuid = this.uuid;
            let that = this;
             this.$axios.post('match/cancelLikeVideo',{"uuid":uuid})
            .then(function (response) {
                if(response.data.succ === 'ok') {
        ...
        comment(e){
            ...
            let that = this;
            this.$axios.post('match/matchVideoComment',{
                 "uuid":uuid,
